//! Vulkan backend: device, the pipelines, the scene's buffers, and a frame
//! rendered either into a window surface (Wayland, X11, Win32, Android) or
//! offscreen for a snapshot. Selected by gpu.zig off the target, and the
//! counterpart of gpu_metal.zig — same scene contract, same blend state, same
//! draw order, so a frame drawn here and a frame drawn there differ only in
//! the API underneath.
//!
//! The conveniences a higher-level API would supply are hand-rolled:
//!   * per-draw uniforms: one host-visible ring buffer + dynamic-offset UBO
//!     descriptor sets in the numbering shaders/vk/* declares (vertex UBO set
//!     1, sampler set 2, fragment UBO set 3), so the shaders are untouched;
//!   * vertex, index and quad buffers: host-visible + coherent, written
//!     directly, so makeScene never touches the queue and can run on the
//!     build worker;
//!   * textures: staging upload and layout transitions on a one-shot command
//!     buffer, fence-waited.
//!
//! Triangles are drawn INDEXED here, unlike the de-indexing older backends
//! did: scene.Range.first/count index the index buffer (shaders/vk/README.md).
//! Single frame in flight: the chart redraws on demand, not at a locked rate,
//! so simplicity beats frame overlap.
...
// wants 4-aligned pCode, so an @alignCast there is UB whenever the linker
// happens to place a blob unaligned.
...
const fill_vert_spv = spvWords(@embedFile("fill_vert_spv"));
const fill_frag_spv = spvWords(@embedFile("fill_frag_spv"));
...
const overlay_vert_spv = spvWords(@embedFile("overlay_vert_spv"));
const overlay_frag_spv = spvWords(@embedFile("overlay_frag_spv"));
...
pub const Options = struct {
    ...
    /// A surface is wanted exactly when a handle was given. gpu_metal.zig
    /// draws the same conclusion, so the two Options agree field for field.
...
pub const Gpu = struct {
    ...
    const VAttr = struct { loc: u32, bind: u32, format: vk.VkFormat, offset: u32 };
    const VBind = struct { bind: u32, stride: u32 };

    // Stream A is the geometry; stream B is evaluated paint, one entry per
    // stream-A vertex, on two parallel buffers so a scene with no
    // zoom-interpolated paint carries no extra bytes.
    const paint_binds = [_]VBind{
        .{ .bind = 1, .stride = @sizeOf(scene.PaintVertex) },
        .{ .bind = 2, .stride = @sizeOf(scene.PaintVertex) },
    };

    // fill: scene.Vertex, stride 28.
    const tri_binds = [_]VBind{.{ .bind = 0, .stride = @sizeOf(scene.Vertex) }} ++ paint_binds;
    const tri_attrs = [_]VAttr{
        .{ .loc = 0, .bind = 0, .format = vk.VK_FORMAT_R32G32_SFLOAT, .offset = 0 }, // a_pos
        .{ .loc = 1, .bind = 0, .format = vk.VK_FORMAT_R32G32_SFLOAT, .offset = 8 }, // a_off
        .{ .loc = 2, .bind = 0, .format = vk.VK_FORMAT_R32_UINT, .offset = 16 }, // a_zwin
        .{ .loc = 3, .bind = 0, .format = vk.VK_FORMAT_R32_UINT, .offset = 20 }, // a_flags
        .{ .loc = 4, .bind = 0, .format = vk.VK_FORMAT_R32_SFLOAT, .offset = 24 }, // a_depth
        .{ .loc = 5, .bind = 1, .format = vk.VK_FORMAT_R8G8B8A8_UNORM, .offset = 0 }, // a_color
        .{ .loc = 6, .bind = 2, .format = vk.VK_FORMAT_R8G8B8A8_UNORM, .offset = 0 }, // a_color_hi
    };

    // pattern: the same geometry, no paint — the cell texture IS the colour.
    const pattern_binds = [_]VBind{.{ .bind = 0, .stride = @sizeOf(scene.Vertex) }};
    const pattern_attrs = tri_attrs[0..5].*;

    // sprite + SDF: scene.Quad, stride 40.
    const quad_binds = [_]VBind{.{ .bind = 0, .stride = @sizeOf(scene.Quad) }} ++ paint_binds;
    const quad_attrs = [_]VAttr{
        .{ .loc = 0, .bind = 0, .format = vk.VK_FORMAT_R32G32_SFLOAT, .offset = 0 }, // a_pos
        .{ .loc = 1, .bind = 0, .format = vk.VK_FORMAT_R32G32_SFLOAT, .offset = 8 }, // a_off
        .{ .loc = 2, .bind = 0, .format = vk.VK_FORMAT_R32G32_SFLOAT, .offset = 16 }, // a_uv
        .{ .loc = 3, .bind = 0, .format = vk.VK_FORMAT_R32_SFLOAT, .offset = 24 }, // a_weight
        .{ .loc = 4, .bind = 0, .format = vk.VK_FORMAT_R32_UINT, .offset = 28 }, // a_zwin
        .{ .loc = 5, .bind = 0, .format = vk.VK_FORMAT_R32_UINT, .offset = 32 }, // a_pack
        .{ .loc = 6, .bind = 0, .format = vk.VK_FORMAT_R32_SFLOAT, .offset = 36 }, // a_depth
        .{ .loc = 7, .bind = 1, .format = vk.VK_FORMAT_R8G8B8A8_UNORM, .offset = 0 }, // a_color
        .{ .loc = 8, .bind = 2, .format = vk.VK_FORMAT_R8G8B8A8_UNORM, .offset = 0 }, // a_color_hi
    };

    // overlay: scene.OverlayVertex, stride 24 — world f2@0, colour f4@8.
    const overlay_binds = [_]VBind{.{ .bind = 0, .stride = @sizeOf(scene.OverlayVertex) }};
    const overlay_attrs = [_]VAttr{
        .{ .loc = 0, .bind = 0, .format = vk.VK_FORMAT_R32G32_SFLOAT, .offset = 0 },
        .{ .loc = 1, .bind = 0, .format = vk.VK_FORMAT_R32G32B32A32_SFLOAT, .offset = 8 },
    };
    ...
    /// covered.
    fn buildPipeline(self: *Gpu, vspv: []const u32, fspv: []const u32, binds: []const VBind, attrs: []const VAttr, depth_write: bool) !vk.VkPipeline {
        ...
        var vbd: [4]vk.VkVertexInputBindingDescription = undefined;
        for (binds, 0..) |b, i| vbd[i] = .{ .binding = b.bind, .stride = b.stride, .inputRate = vk.VK_VERTEX_INPUT_RATE_VERTEX };
        var vat: [12]vk.VkVertexInputAttributeDescription = undefined;
        for (attrs, 0..) |a, i| vat[i] = .{ .location = a.loc, .binding = a.bind, .format = a.format, .offset = a.offset };
        ...
        vi.vertexBindingDescriptionCount = @intCast(binds.len);
    ...
    fn createPipelines(self: *Gpu) !void {
        self.pipeline = try self.buildPipeline(fill_vert_spv, fill_frag_spv, &tri_binds, &tri_attrs, false);
        self.pattern_pipeline = try self.buildPipeline(pattern_vert_spv, pattern_frag_spv, &pattern_binds, &pattern_attrs, false);
        self.sprite_pipeline = try self.buildPipeline(sprite_vert_spv, sprite_frag_spv, &quad_binds, &quad_attrs, false);
        self.sdf_pipeline = try self.buildPipeline(sprite_vert_spv, sdf_frag_spv, &quad_binds, &quad_attrs, false);
        ...
        self.overlay_pipeline = try self.buildPipeline(overlay_vert_spv, overlay_frag_spv, &overlay_binds, &overlay_attrs, false);
    }
    ...
    fn resizeFallible(self: *Gpu, width_pts: u32, height_pts: u32) !void {
    ...
    /// Replace a horizontal band of the sprite atlas in place. The symbol
    /// atlas grows a row at a time as new symbols bake, and re-uploading the
    ...
    pub fn updateSpriteAtlasRows(self: *Gpu, rgba: []const u8, w: u32, h: u32, y0: u32, rows: u32) bool {
        ...
        var staging = self.createBuffer(need, vk.VK_BUFFER_USAGE_TRANSFER_SRC_BIT, true) catch return false;
        defer self.destroyBuffer(&staging);
        @memcpy(staging.mapped.?[0..need], rgba[off .. off + need]);

        const Ctx = struct { img: vk.VkImage, w: u32, y0: u32, rows: u32, buf: vk.VkBuffer };
        self.oneShot(Ctx{ .img = t.img, .w = w, .y0 = y0, .rows = rows, .buf = staging.buf }, struct {
            fn rec(ctx: Ctx, cmd: vk.VkCommandBuffer) void {
                ...
                bar.srcAccessMask = vk.VK_ACCESS_SHADER_READ_BIT;
                bar.dstAccessMask = vk.VK_ACCESS_TRANSFER_WRITE_BIT;
                // The atlas has been sampled since it was made, so it comes in
                // readable, not undefined — preserving it is the whole point.
                bar.oldLayout = vk.VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
                ...
                vk.vkCmdPipelineBarrier(cmd, vk.VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, vk.VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, null, 0, null, 1, &bar);
                ...
                region.imageOffset = .{ .x = 0, .y = @intCast(ctx.y0), .z = 0 };
                region.imageExtent = .{ .width = ctx.w, .height = ctx.rows, .depth = 1 };
    ...
    /// The host's own geometry, drawn after the scene in the same pass so the
    ...
    fn makeSceneHeld(self: *Gpu, alloc: std.mem.Allocator, data: SceneData) !Scene {
        ...
        if (data.vertices.len > 0 and data.indices.len > 0) {
            out.vbuf = try self.upload(std.mem.sliceAsBytes(data.vertices), vk.VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
            out.ibuf = try self.upload(std.mem.sliceAsBytes(data.indices), vk.VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
            if (data.paint.len > 0)
                out.pbuf = try self.upload(std.mem.sliceAsBytes(data.paint), vk.VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
            ...
            if (data.paint_hi.len > 0)
                out.phbuf = try self.upload(std.mem.sliceAsBytes(data.paint_hi), vk.VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
        }
        if (data.quads.len > 0) {
            out.qbuf = try self.upload(std.mem.sliceAsBytes(data.quads), vk.VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
            if (data.quad_paint.len > 0)
                out.qpbuf = try self.upload(std.mem.sliceAsBytes(data.quad_paint), vk.VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
        }
    ...
    fn upload(self: *Gpu, bytes: []const u8, usage: vk.VkBufferUsageFlags) !Buffer {
        var b = try self.createBuffer(bytes.len, usage, true);
    ...
    fn recordDraws(self: *Gpu, cmd: vk.VkCommandBuffer, pass: vk.VkRenderPass, fb: vk.VkFramebuffer, u: Uniforms) void {
        self.ring_off = 0;
    ...
    /// The chart itself, in paint order.
    fn recordScene(self: *Gpu, cmd: vk.VkCommandBuffer, u: Uniforms) void {
        ...
        // The batcher folds contiguous ranges sharing a spec into one call. That
        // matters: a coastal view carries thousands of ranges, and binding each
        // one separately dominated the frame. This loop binds and draws what it
        ...
        for (self.batchScene(s)) |d| {
            ...
            if (tri) {
                if (s.vbuf.buf == null or s.ibuf.buf == null) continue;
                pipe = self.pipeline;
            ...
            if (bound_prim == null or bound_prim.? != d.prim) {
                if (tri) {
                    // Stream B rides parallel buffers; a scene with no
                    // zoom-interpolated paint binds the first one twice, and the
                    // shader's mix collapses to a no-op.
                    const pb = if (s.pbuf.buf != null) s.pbuf.buf else s.vbuf.buf;
                    const ph = if (s.phbuf.buf != null) s.phbuf.buf else pb;
                    const bufs = [3]vk.VkBuffer{ s.vbuf.buf, pb, ph };
                    const offs = [3]u64{ 0, 0, 0 };
                    vk.vkCmdBindVertexBuffers(cmd, 0, 3, &bufs, &offs);
                    vk.vkCmdBindIndexBuffer(cmd, s.ibuf.buf, 0, vk.VK_INDEX_TYPE_UINT32);
                } else {
                    const qp = if (s.qpbuf.buf != null) s.qpbuf.buf else s.qbuf.buf;
                    const bufs = [3]vk.VkBuffer{ s.qbuf.buf, qp, qp };
                    const offs = [3]u64{ 0, 0, 0 };
                    vk.vkCmdBindVertexBuffers(cmd, 0, 3, &bufs, &offs);
                }
                bound_prim = d.prim;
    ...
    fn renderWindowFallible(self: *Gpu, u: Uniforms) !bool {
        ...
        self.recordDraws(self.cmd, self.render_pass, self.sc_fbs[image_index], u);
    ...
    pub fn renderOffscreen(self: *Gpu, alloc: std.mem.Allocator, u: Uniforms) ![]u8 {
        ...
        bi.flags = vk.VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
        try check(vk.vkBeginCommandBuffer(self.cmd, &bi), "begin cmd");
        self.recordDraws(self.cmd, self.off_pass, self.off_fb, u);
